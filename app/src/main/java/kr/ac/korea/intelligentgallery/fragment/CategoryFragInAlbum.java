...
//            DebugUtil.showDebug(TAG + "doInBackground::");
            ...
            if (imagesInFolder != null) {
                ...
                tempCategories = DatabaseCRUD.selectCategoryFragInAlbumCategoryList(imagesInFolder);
                ...
                if (categories != null && categories.size() > 0) {
                    ...
                    while (categories.size() != 0) {
                        ...
                        String cName = DiLabClassifierUtil.cNameConvertWrapper(cNameOriginal); // 임시코드
//                        String cName = DiLabClassifierUtil.cNameConverter.convert(cNameOriginal); // 원래 사용했던 코드
                        ...
                        int redundentCatIndex = 0;
//                        for (Category newcat : newCategories) {
//                            if(newcat.getcID() == currentCID) { isNewCategory = false; }
//                        }
                        for (int k=0; k < newCategories.size(); k++) {
                            if(newCategories.get(k).getcID() == currentCID) {
                                isNewCategory = false;
                                redundentCatIndex = k;
                            }
                        }
//                        Log.i(TAG, "cID:" + currentCID + ", cName:" + cName + ", isNewCategory:" + isNewCategory);
                        ...
                        if(isNewCategory) {
                            ...
                            ArrayList<ImageFile> addedImageFiles = new ArrayList<>();
                            addedImageFiles.addAll(newCategories.get(redundentCatIndex).getContainingImages());
                            addedImageFiles.add(categories.get(i).getContainingImages().get(0));

                            newCategories.get(redundentCatIndex).setContainingImages(addedImageFiles);
                            newCategories.get(redundentCatIndex).setCount(newCategories.get(redundentCatIndex).getCount()+1);
                        }
